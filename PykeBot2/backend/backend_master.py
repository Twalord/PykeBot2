...
import logging
import asyncio
from PykeBot2.models.data_models import (
    Error,
    Message,
    Payload,
    Team,
...
from PykeBot2.backend.stalker import op_gg_rank, prime_league, battlefy
from PykeBot2.backend.stalker import (
    toornament_api,
    riot_api_rank,
    toornament,
    summoners_inn,
...
from PykeBot2.models.lookup_tables import (
    prime_league_base_url,
    prime_league_group_key_words,
    prime_league_season_key_words,
    prime_league_team_key_words,
    toornament_base_url,
    toornament_tournament_key_words,
    with_ranks_flag_lookup,
    summoners_inn_base_url,
    summoners_inn_cup_key_words,
    summoners_inn_team_key_words,
    battlefy_base_url,
...
"""
website_type_to_prime_league_stalker = {
    "group": prime_league.stalk_prime_league_group,
    "team": prime_league.stalk_prime_league_team,
    "season": prime_league.stalk_prime_league_season,
}

website_type_to_toornament_stalker = {
    "tournament": toornament.stalk_toornament_tournament,
    "tournament_api": toornament_api.stalk_toornament_api_tournament,
}

website_type_to_summoners_inn_stalker = {
    "cup": summoners_inn.stalk_summoners_inn_cup,
    "team": prime_league.stalk_prime_league_team,
}
...
def determine_stalker(query: Query):
    ...
    if website == prime_league_str:
        ...
        # check season last as its keywords are a subset of the other sets
            ...
            error_message = (
                f"Invalid url for query {str(query)}, url {url} was matched with prime league but no "
    ...
    if website == toornament_str:
        if all(elem in url.split("/") for elem in toornament_tournament_key_words):
            ...
            error_message = (
                f"Invalid url for query {str(query)}, url {url} was matched with toornament but no "
        ...
        if (
            ...
            and not len(dont_use_api_flag_lookup.intersection(query.flags)) >= 1
        ):
    ...
    if website == summoners_inn_str:
        if all(elem in url.split("/") for elem in summoners_inn_team_key_words):
            ...
            error_message = (
                f"Invalid url for query {str(query)}, url {url} was matched with summoners-inn but no "
                f"tournament was found. "
            )
            logger.error(error_message)
            create_error(query, error_message)
